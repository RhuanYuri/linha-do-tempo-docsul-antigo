	...
	function TouchSwipe(element, options) {
		...
		*/
		function touchStart(jqEvent) {
			...
			/  we use Jquery bind for events, we need to target the original event object
			...
			if (!touches || (fingerCount === options.fingers || options.fingers === ALL_FINGERS) || hasPinches()) {
				...
				/ touch with more or less than the fingers we are looking for, so cancel
				ret = false; 
			}
...
(function(window, moduleName) {
		...
		var _namespace = function(ns) {
			...
			/ quick way to create a class that doesn't have any dependencies. Returns the class, but first registers it in the GreenSock namespace so that other classes can grab it (other classes might be dependent on the class).
 ...
 */
		...
		_class("Ticker", function(fps, useRAF) {
			...
			/ bug in iOS 6 Safari occasionally prevents the requestAnimationFrame from working initially, so we use a 1.5-second timeout that automatically falls back to setTimeout() if it senses this condition.
			setTimeout(function() {
			...
			_addPropTween = function(target, prop, start, end, overwriteProp, round, funcParam, stringFilter) {
				...
				if (type !== "number") {
					...
					if (typeof(s) === "string" && (funcParam || isNaN(s))) {
						/ blob (string that has multiple numbers in it)
						pt.fp = funcParam;
